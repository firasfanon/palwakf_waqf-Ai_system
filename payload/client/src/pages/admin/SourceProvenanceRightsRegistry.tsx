  ...
  return (
    <AdminPage className="source-provenance-rights-registry">
      <div className="space-y-3">
        ...
        <AdminCard title="AR1 — Pilot Agentic RAG داخلي محكوم" description="Vertical Slice داخلي فقط: اختيار corpus صغير من cohort C4، سؤال بحثي مقيد، استشهادات ظاهرة، وامتناع/تصعيد عند نقص الدليل. لا يكتب أي مصدر أو حق أو chunks أو vectors، ولا يفتح Chat العام.">
          <div className="space-y-5">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
              <div className="flex items-start gap-3 text-sm leading-7 text-muted-foreground"><Bot className="mt-1 h-5 w-5 shrink-0 text-primary" /><p>تُنشأ الجلسة في ذاكرة الخادم لمدة محدودة ثم يمكن إلغاؤها. مستويات T3/T4 هنا إقرار تشغيلي مؤقت من المراجع ولا تعدّل حالة الحقوق أو المصدر في قاعدة البيانات.</p></div>
              <div className="flex flex-wrap gap-2">
                <Badge variant={pilotStatus.data?.status === 'READY_FOR_OPERATOR_BINDING' ? 'secondary' : 'outline'}>{pilotStatus.data?.status || 'جارٍ فحص البوابة'}</Badge>
                <Badge variant="outline">LLM: {pilotStatus.data?.llmGenerationEnabled ? 'مفعّل داخليًا' : 'Evidence-only'}</Badge>
              </div>
            </div>

            {pilotStatus.isLoading || pilotCandidates.isLoading ? <p className="text-sm text-muted-foreground">جارٍ قراءة cohort C4 والمواد المرتبطة دون تشغيل أي فحص خارجي…</p> : null}
            {pilotStatus.error || pilotCandidates.error ? <div role="alert" className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">تعذر قراءة بوابة AR1: {pilotStatus.error?.message || pilotCandidates.error?.message}</div> : null}
            {pilotStatus.data?.reason ? <div className="rounded-lg border border-amber-500/40 bg-amber-500/5 p-3 text-sm leading-7 text-muted-foreground"><LockKeyhole className="ml-2 inline h-4 w-4 text-amber-600" />{pilotStatus.data.reason}</div> : null}
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-lg border p-3 text-sm"><div className="text-muted-foreground">مواد C4 القابلة للربط</div><div className="mt-1 text-2xl font-bold">{pilotStatus.data?.availableCandidateDocuments ?? 0}</div></div>
              <div className="rounded-lg border p-3 text-sm"><div className="text-muted-foreground">حد الوثائق للجلسة</div><div className="mt-1 text-2xl font-bold">{pilotStatus.data?.maxBindings ?? 5}</div></div>
              <div className="rounded-lg border p-3 text-sm"><div className="text-muted-foreground">مدة الجلسة</div><div className="mt-1 text-2xl font-bold">{pilotStatus.data?.maxSessionMinutes ?? 30} د</div></div>
              <div className="rounded-lg border p-3 text-sm"><div className="text-muted-foreground">الجلسات النشطة</div><div className="mt-1 text-2xl font-bold">{pilotStatus.data?.activeSessions ?? 0}</div></div>
            </div>

            {!pilotSession ? <div className="space-y-4 rounded-xl border border-border/70 bg-muted/20 p-4">
              <div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" /><h3 className="font-semibold">1. ربط corpus تجريبي داخلي</h3></div>
              <p className="text-xs leading-6 text-muted-foreground">اختر فقط مواد C4 المباشرة أو المواد التي حُلّت بمطابقة حتمية حالية للعنوان أو الرابط. لا تُنشئ المطابقة رابط مصدر دائمًا ولا تعيّن حقًا قانونيًا؛ يلزم مرجع اعتماد الاستخدام الداخلي لكل وثيقة.</p>
              <div className="space-y-3">
                {pilotCandidateRows.map((candidate: any) => {
                  ...
                  return <div className="rounded-lg border bg-background p-3" key={key}>
                ...
                {!pilotCandidateRows.length && !pilotCandidates.isLoading ? <p className="text-sm text-muted-foreground">لا توجد مادة knowledge_document مباشرة أو مطابقة حتميًا بعنوان/رابط حالي لـ cohort C4 في هذه الجولة. شغّل C3 ثم C4 من جديد، أو راجع العنوان والرابط دون استخدام مطابقة تقريبية.</p> : null}
              </div>
              ...
              <Button onClick={() => startPilotSession.mutate({ bindings: selectedPilotBindings, rightsApprovalRef: pilotRightsApprovalRef, reviewerNote: pilotReviewerNote || null, acknowledgeInternalOnly: true, acknowledgeModelBoundary: true })} disabled={pilotStatus.data?.status !== 'READY_FOR_OPERATOR_BINDING' || !selectedPilotBindings.length || !pilotRightsApprovalRef.trim() || selectedPilotBindings.some((binding) => !binding.internalUseApprovalRef.trim()) || !pilotInternalOnlyAcknowledged || !pilotModelBoundaryAcknowledged || startPilotSession.isPending}>
                <Play className="ml-2 h-4 w-4" />{startPilotSession.isPending ? 'جارٍ إنشاء الجلسة…' : 'إنشاء جلسة AR1 الداخلية'}
              </Button>
            </div> : <div className="space-y-4 rounded-xl border border-primary/30 bg-primary/5 p-4">
              <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><div className="font-semibold">2. جلسة AR1 نشطة</div><div className="mt-1 text-xs text-muted-foreground">{pilotSession.id} · تنتهي: {dateLabel(pilotSession.expiresAt)} · {pilotSession.bindingCount} مواد</div></div><Button variant="outline" onClick={() => rollbackPilot.mutate({ sessionId: pilotSession.id })} disabled={rollbackPilot.isPending}><RotateCcw className="ml-2 h-4 w-4" />إلغاء الجلسة ومسح الذاكرة</Button></div>
