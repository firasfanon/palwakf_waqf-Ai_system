...
const workspaces = [
  ...
  },
  {
    ...
    href: APP_ROUTES.adminKnowledgeReviewOperations,
    icon: ListChecks,
  ...
  },
  {
    ...
    href: APP_ROUTES.adminToolsOutputIntake,
    icon: Sparkles,
    lifecycle: "مخرج قابل للمراجعة",
  },
  {
    title: "معاينة جرد Manus",
    description: "فحص ملف المصادر والحقوق محليًا قبل المطابقة أو الاستيراد.",
    href: APP_ROUTES.adminSourceInventoryPreview,
    icon: FileSearch,
    lifecycle: "Preview only / no write",
  },
  {
...
export default function KnowledgeOperationsWorkspace() {
  ...
  const summary = useMemo(() => ({
    sources: Number(sourceStats.data?.totalSources || 0),
  ...
  }), [snapshot.data, sourceStats.data?.totalSources]);
  ...
  return (
    <AdminPage className="knowledge-operations-workspace">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <AdminCard title="المصادر الأصلية"><div className="text-3xl font-bold">{summary.sources}</div><p className="mt-2 text-sm text-muted-foreground">مصدر مسجل في طبقة المصادر التشغيلية.</p></AdminCard>
