...
export default function SourceProvenanceRightsRegistry() {
  ...
  const [pilotNoRightsGrantAcknowledged, setPilotNoRightsGrantAcknowledged] = useState(false);
  const [pilotModelBoundaryAcknowledged, setPilotModelBoundaryAcknowledged] = useState(false);
  const [pilotSessionStartReference, setPilotSessionStartReference] = useState('');
  const [pilotEvidenceOnlyStartAcknowledged, setPilotEvidenceOnlyStartAcknowledged] = useState(false);
  const [pilotOneQuestionLimitAcknowledged, setPilotOneQuestionLimitAcknowledged] = useState(false);
  const [pilotRollbackRequiredAcknowledged, setPilotRollbackRequiredAcknowledged] = useState(false);
  const [pilotAuthorityPreparation, setPilotAuthorityPreparation] = useState<any>(null);
  ...
  const [singleFlowDetailsOpen, setSingleFlowDetailsOpen] = useState(false);
  ...
  const singleFlowReadiness = trpc.agenticRagPilot.runSingleFlowReadinessCycle.useMutation({
    onSuccess: async (data) => {
      setPilotSelection({});
      setPilotAuthorityPreparation(null);
      setPilotResult(null);
      await Promise.all([
        utils.agenticRagPilot.status.invalidate(),
        utils.agenticRagPilot.candidates.invalidate(),
        utils.sourceProvenance.controlledReleasePlan.invalidate(),
      ]);
      if (data.c4.status === 'READY_FOR_OPERATOR_BINDING') {
        toast.success('اكتملت دورة التحقق وعُثر على مادة قابلة للمراجعة.', { description: 'اختر مادة واحدة فقط قبل ظهور اعتماد الجلسة.' });
      } else {
        toast.message('اكتملت دورة التحقق دون مرشح قابل للربط.', { description: 'لم تُنشأ أي جلسة أو كتابة دائمة.' });
      }
    },
    onError: (error) => toast.error('تعذر إكمال دورة التحقق', { description: error.message }),
  });

  const pilotStatusCode = singleFlowReadiness.data?.c4?.status || pilotStatus.data?.status || '';
  const pilotCandidateRows = singleFlowReadiness.data?.candidates || pilotCandidates.data?.candidates || [];
  const pilotCandidateCount = singleFlowReadiness.data?.c4?.candidateCount ?? pilotStatus.data?.availableCandidateDocuments ?? pilotCandidateRows.length;
  const c3C4EvidenceReady = singleFlowReadiness.data?.c3?.status === 'completed'
    || (Boolean(pilotStatusCode) && pilotStatusCode !== 'C3_C4_FRESH_EVIDENCE_REQUIRED_HOLD');
  ...
  const pilotAuthorityPreparationReady = pilotMaterialReady
    && selectedPilotBindings.length === 1
    && Boolean(pilotRightsApprovalRef.trim())
    && Boolean(selectedPilotBindings[0]?.internalUseApprovalRef.trim())
    && pilotReviewerNote.trim().length >= 12
    && pilotInternalOnlyAcknowledged
    && pilotNoRightsGrantAcknowledged
    && pilotModelBoundaryAcknowledged;
  const pilotSessionStartReady = pilotAuthorityPreparation?.status === 'prepared'
    && pilotSessionStartReference.trim().length >= 3
    && pilotEvidenceOnlyStartAcknowledged
    && pilotOneQuestionLimitAcknowledged
    && pilotRollbackRequiredAcknowledged
    && !pilotSession;
  const singleFlowReason = singleFlowReadiness.data?.c4?.reason || pilotStatus.data?.reason || 'اضغط بدء دورة التحقق لتشغيل C3 ثم قراءة C4 وفحص أهلية المادة ضمن العملية الخادمية نفسها.';
  const singleFlowNextAction = singleFlowReadiness.data?.nextAction || null;
  ...
  const preparePilotSessionAuthority = trpc.agenticRagPilot.prepareSessionAuthority.useMutation({
    onSuccess: (data) => {
      ...
      toast.success('تم تحضير صلاحية جلسة AR1 المؤقتة دون بدء الجلسة.', { description: 'التحضير مؤقت ومقيد بالمشغّل ودليل C3 الحالي، ولا يمنح حقوقًا دائمة.' });
      void pilotStatus.refetch();
    },
    onError: (error) => toast.error('تعذر تحضير صلاحية جلسة AR1', { description: error.message }),
  });

  const revokePilotSessionAuthority = trpc.agenticRagPilot.revokeAuthorityPreparation.useMutation({
    onSuccess: () => {
      setPilotAuthorityPreparation(null);
      toast.success('تم إلغاء تحضير الصلاحية ومسحه من ذاكرة الخادم.');
      void pilotStatus.refetch();
    },
    onError: (error) => toast.error('تعذر إلغاء تحضير الصلاحية', { description: error.message }),
  });

  const startPilotSession = trpc.agenticRagPilot.startSession.useMutation({
    onSuccess: (data) => {
      setPilotSession(data);
      setPilotAuthorityPreparation((current: any) => current ? {
        ...current,
        status: 'consumed',
        sessionStarted: true,
        sessionId: data.id,
        nextAction: 'EPHEMERAL_SESSION_STARTED',
      } : current);
      setPilotResult(null);
      toast.success('تم إنشاء جلسة AR1 Evidence-only المؤقتة.', { description: 'الجلسة تسمح بسؤال واحد فقط، ولا تستخدم LLM ولا تكتب أي حالة دائمة.' });
      void pilotStatus.refetch();
    },
  ...
  const askPilot = trpc.agenticRagPilot.ask.useMutation({
    onSuccess: (data) => {
      setPilotResult(data);
      setPilotSession((current: any) => current ? {
        ...current,
        questionsExecuted: data.questionCount ?? 1,
        llmGenerationUsed: data.llmGenerationUsed === true,
      } : current);
      toast.success(data.action === 'answer' ? 'اكتملت استجابة Evidence-only المقيدة بالاستشهادات.' : 'تم تطبيق بوابة الامتناع أو التصعيد.');
    },
  ...
  const rollbackPilot = trpc.agenticRagPilot.rollbackSession.useMutation({
    onSuccess: () => {
      ...
      setPilotResult(null);
      setPilotQuestion('');
      setPilotSessionStartReference('');
      setPilotEvidenceOnlyStartAcknowledged(false);
      setPilotOneQuestionLimitAcknowledged(false);
      setPilotRollbackRequiredAcknowledged(false);
      toast.success('تمت إزالة جلسة AR1 من ذاكرة الخادم وإغلاق Runtime UAT.');
      void pilotStatus.refetch();
    },
  ...
  const updatePilotSelection = (candidate: any, patch: Partial<PilotSelection>) => {
    ...
    setPilotSelection((previous) => {
      const next = Object.fromEntries(Object.entries(previous).map(([existingKey, value]) => [
        existingKey,
        patch.selected === true && existingKey !== key ? { ...value, selected: false } : value,
      ])) as Record<string, PilotSelection>;
      next[key] = {
        ...
        >
          <div className="space-y-5">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
              <div className="flex items-start gap-3 text-sm leading-7 text-muted-foreground"><Bot className="mt-1 h-5 w-5 shrink-0 text-primary" /><p>مسار واحد للمشغّل: تبدأ الدورة من هنا، ثم ينفذ النظام C3 وقراءة C4 وفحص الأهلية الحتمية داخل العملية الخادمية نفسها. لا تُكتب مصادر أو حقوق أو وثائق، ولا يُفتح Chat العام.</p></div>
              <div className="flex flex-wrap gap-2"><Badge variant="outline">داخلي فقط</Badge><Badge variant="outline">LLM: {pilotStatus.data?.llmGenerationEnabled ? 'مفعّل داخليًا' : 'Evidence-only'}</Badge></div>
            </div>

            {pilotStatus.error || pilotCandidates.error ? <div role="alert" className="rounded-lg border border-destructive/40 bg-destructive/5 p-4 text-sm leading-7 text-destructive">تعذر قراءة حالة AR1. لا تبدأ جلسة قبل عودة القراءة: {pilotStatus.error?.message || pilotCandidates.error?.message}</div> : null}

            {!pilotSession ? <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-start gap-3"><Play className="mt-1 h-5 w-5 shrink-0 text-primary" /><div><h3 className="font-semibold">دورة التحقق التجريبي</h3><p className="mt-1 text-sm leading-7 text-muted-foreground">اضغط زرًا واحدًا فقط. سيجري النظام فحص C3 الخارجي المحدود، ثم قراءة C4، ثم يبحث عن مادة مرتبطة بمطابقة حتمية فقط.</p></div></div>
                <Button size="lg" onClick={() => singleFlowReadiness.mutate({ maxUrls: 32 })} disabled={singleFlowReadiness.isPending}>
                  {singleFlowReadiness.isPending ? <RefreshCw className="ml-2 h-4 w-4 animate-spin" /> : <Play className="ml-2 h-4 w-4" />}
                  {singleFlowReadiness.isPending ? 'جارٍ تنفيذ دورة التحقق…' : 'بدء دورة التحقق'}
                </Button>
              </div>
              <p className="mt-3 text-xs leading-6 text-muted-foreground">الفحص صريح بطلب المشغّل، محدود بالبيانات الوصفية، ولا يتبع التحويلات أو يقرأ النصوص أو PDF.</p>
            </div> : null}

            {singleFlowReadiness.error ? <div role="alert" className="rounded-xl border border-destructive/40 bg-destructive/5 p-4 text-sm leading-7 text-destructive">تعذر إكمال الدورة. لم تُنشأ جلسة ولم تُكتب بيانات: {singleFlowReadiness.error.message}</div> : null}

            {!pilotSession && singleFlowReadiness.data ? <div className="rounded-2xl border p-5">
              {pilotMaterialReady ? <div className="flex items-start gap-3"><ShieldCheck className="mt-1 h-5 w-5 shrink-0 text-primary" /><div><h3 className="font-semibold">تم العثور على مادة قابلة للربط</h3><p className="mt-1 text-sm leading-7 text-muted-foreground">اختر مادة واحدة فقط أدناه. بعد المراجعة تُحضّر صلاحية مؤقتة مرتبطة بالمشغّل؛ لا تبدأ الجلسة تلقائيًا.</p></div></div> : <div className="flex items-start gap-3"><LockKeyhole className="mt-1 h-5 w-5 shrink-0 text-muted-foreground" /><div><h3 className="font-semibold">لم يتم العثور على مادة قابلة للربط حتميًا</h3><p className="mt-1 text-sm leading-7 text-muted-foreground">لا يلزم منك إجراء إضافي داخل هذه الصفحة. بقي AR1 مقفلًا كما يجب، ولم تُنشأ أي جلسة أو كتابة دائمة.</p></div></div>}
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div className="rounded-lg border bg-muted/20 p-3 text-sm"><div className="text-muted-foreground">فحوص C3 المنفذة</div><div className="mt-1 text-xl font-bold">{singleFlowReadiness.data.c3.requestedChecks}</div></div><div className="rounded-lg border bg-muted/20 p-3 text-sm"><div className="text-muted-foreground">مواد C4 القابلة للربط</div><div className="mt-1 text-xl font-bold">{pilotCandidateCount}</div></div><div className="rounded-lg border bg-muted/20 p-3 text-sm"><div className="text-muted-foreground">الخطوة التالية</div><div className="mt-1 text-sm font-semibold">{singleFlowNextAction === 'REVIEW_ONE_DETERMINISTIC_CANDIDATE' ? 'مراجعة مادة واحدة' : 'لا توجد خطوة تشغيلية'}</div></div></div>
            </div> : null}

            {!pilotSession && !singleFlowReadiness.data ? <div className="rounded-xl border border-dashed bg-muted/20 p-4 text-sm leading-7 text-muted-foreground">لم تبدأ دورة تحقق في هذه الجلسة بعد. لا تحتاج إلى فتح C3 أو C4 أو التنقل بين أقسام الصفحة؛ استخدم زر <strong>بدء دورة التحقق</strong> أعلاه.</div> : null}

            <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-4"><div><div className="font-medium">تفاصيل الحوكمة والتدقيق</div><div className="mt-1 text-xs text-muted-foreground">تفاصيل ثانوية للمراجعة؛ ليست خطوة تشغيلية.</div></div><Button variant="ghost" size="sm" onClick={() => setSingleFlowDetailsOpen((value) => !value)}>{singleFlowDetailsOpen ? 'إخفاء التفاصيل' : 'عرض التفاصيل'} {singleFlowDetailsOpen ? <ChevronUp className="mr-2 h-4 w-4" /> : <ChevronDown className="mr-2 h-4 w-4" />}</Button></div>
            {singleFlowDetailsOpen ? <div className="rounded-xl border bg-muted/20 p-4 text-xs leading-7 text-muted-foreground"><div>حالة AR1: <code>{pilotStatusCode || 'not_started'}</code></div><div>نتيجة الأهلية: {singleFlowReason}</div><div>مطابقات مباشرة/مرجعية/دقيقة: <code>{pilotStatus.data?.corpusResolution?.directC4MaterialReferences ?? 0}</code> / <code>{pilotStatus.data?.corpusResolution?.directReferenceDocumentAssociations ?? 0}</code> / <code>{pilotStatus.data?.corpusResolution?.deterministicExactMatches ?? 0}</code></div><div>لا تستخدم الدورة مطابقة تقريبية أو دلالية أو Vector أو الكاتب/الناشر فقط.</div></div> : null}

            {!pilotSession && pilotMaterialReady ? <div className="space-y-4 rounded-2xl border border-border/70 bg-muted/20 p-4">
              <div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" /><div><h3 className="font-semibold">مراجعة مرشح واحد وتحضير صلاحية الجلسة</h3><p className="mt-1 text-xs leading-6 text-muted-foreground">التحضير لا يبدأ AR1 ولا ينشئ حقًا أو رابط مصدر. يُصدر تصريحًا مؤقتًا في ذاكرة الخادم، مرتبطًا بالمشغّل وبصلاحية دليل C3 الحالي.</p></div></div>
              <div className="space-y-3">
                {pilotCandidateRows.map((candidate: any) => {
                  ...
                  return <div className="rounded-xl border bg-background p-3" key={key}>
                    ...
                    {selection.selected ? <div className="mt-3 grid gap-3 md:grid-cols-2"><div><Label>درجة الدليل للتحضير</Label><Select value={selection.evidenceTier} onValueChange={(value) => updatePilotSelection(candidate, { evidenceTier: value as PilotSelection['evidenceTier'] })}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="T3_CONTROLLED_INTERNAL_EVIDENCE">T3 — دليل داخلي مضبوط</SelectItem><SelectItem value="T4_VERIFIED_CITATION_EVIDENCE">T4 — دليل استشهاد متحقق</SelectItem></SelectContent></Select></div><div><Label>مرجع اعتماد الاستخدام الداخلي</Label><Input className="mt-1" value={selection.internalUseApprovalRef} onChange={(event) => updatePilotSelection(candidate, { internalUseApprovalRef: event.target.value })} placeholder="مثال: AR1-INT-APPROVAL-001" /></div></div> : null}
                  </div>;
                })}
              </div>
              {selectedPilotBindings.length === 1 ? <>
                <div className="grid gap-3 md:grid-cols-2"><div><Label>مرجع اعتماد الحقوق/الاستخدام للتحضير</Label><Input className="mt-1" value={pilotRightsApprovalRef} onChange={(event) => { setPilotRightsApprovalRef(event.target.value); setPilotAuthorityPreparation(null); }} placeholder="مرجع قرار داخلي قابل للتتبع" /></div><div><Label>مبررات اختيار المرشح (إلزامية)</Label><Textarea className="mt-1" value={pilotReviewerNote} onChange={(event) => { setPilotReviewerNote(event.target.value); setPilotAuthorityPreparation(null); }} placeholder="اشرح سبب اختيار المادة وحدود الاستخدام الداخلي بما لا يقل عن 12 حرفًا" /></div></div>
                <div className="space-y-2 text-sm"><label className="flex items-start gap-3"><Checkbox checked={pilotInternalOnlyAcknowledged} onCheckedChange={(checked) => { setPilotInternalOnlyAcknowledged(checked === true); setPilotAuthorityPreparation(null); }} /><span>أقر أن التحضير للبحث الداخلي فقط، وليس نشرًا عامًا أو Chat عامًا أو قرارًا قانونيًا.</span></label><label className="flex items-start gap-3"><Checkbox checked={pilotNoRightsGrantAcknowledged} onCheckedChange={(checked) => { setPilotNoRightsGrantAcknowledged(checked === true); setPilotAuthorityPreparation(null); }} /><span>أقر أن هذا التحضير لا يمنح ترخيصًا ولا يعيّن حقوقًا ولا ينشئ رابط مصدر دائمًا.</span></label><label className="flex items-start gap-3"><Checkbox checked={pilotModelBoundaryAcknowledged} onCheckedChange={(checked) => { setPilotModelBoundaryAcknowledged(checked === true); setPilotAuthorityPreparation(null); }} /><span>أقر أن مزود النموذج المحلي أو الداخلي المعتمد فقط هو المسموح به عند أي تفعيل لاحق.</span></label></div>
                {!pilotAuthorityPreparation ? <div className="flex flex-wrap items-center gap-3"><Button onClick={() => preparePilotSessionAuthority.mutate({ binding: selectedPilotBindings[0], rightsApprovalRef: pilotRightsApprovalRef, reviewRationale: pilotReviewerNote, reviewDecision: 'APPROVE_EPHEMERAL_INTERNAL_SESSION_PREP', acknowledgeInternalOnly: true, acknowledgeNoRightsGrant: true, acknowledgeModelBoundary: true })} disabled={!pilotAuthorityPreparationReady || preparePilotSessionAuthority.isPending}><ShieldCheck className="ml-2 h-4 w-4" />{preparePilotSessionAuthority.isPending ? 'جارٍ تحضير الصلاحية…' : 'تحضير صلاحية جلسة AR1'}</Button><span className="text-xs text-muted-foreground">{pilotAuthorityPreparationReady ? 'المدخلات مكتملة للتحضير فقط.' : 'أكمل المرجعين والمبررات والإقرارات قبل التحضير.'}</span></div> : null}
                {pilotAuthorityPreparation ? <div className="space-y-3 rounded-xl border border-primary/30 bg-primary/5 p-4">
                  <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                    <div>
                      <div className="font-semibold">{pilotAuthorityPreparation.status === 'prepared' ? 'تم تحضير الصلاحية دون بدء الجلسة' : 'تم استهلاك الصلاحية لبدء جلسة Evidence-only'}</div>
                      <div className="mt-1 text-xs leading-6 text-muted-foreground"><code>{pilotAuthorityPreparation.id}</code> · تنتهي: {dateLabel(pilotAuthorityPreparation.expiresAt)} · fingerprint: <code>{pilotAuthorityPreparation.authorityFingerprint}</code></div>
                    </div>
                    {pilotAuthorityPreparation.status === 'prepared' ? <Button variant="outline" onClick={() => revokePilotSessionAuthority.mutate({ authorityPreparationId: pilotAuthorityPreparation.id })} disabled={revokePilotSessionAuthority.isPending}><RotateCcw className="ml-2 h-4 w-4" />إلغاء التحضير</Button> : null}
                  </div>
                  <div className="grid gap-2 text-xs md:grid-cols-2">
                    <div>الحالة: <code>{pilotAuthorityPreparation.status}</code></div>
                    <div>الجلسة بدأت: <code>{String(pilotAuthorityPreparation.sessionStarted)}</code></div>
                    <div>كتابات قاعدة البيانات: <code>{pilotAuthorityPreparation.databaseWrites}</code></div>
                    <div>الخطوة التالية: <code>{pilotAuthorityPreparation.nextAction}</code></div>
                  </div>
                  {pilotAuthorityPreparation.status === 'prepared' ? <div className="space-y-3 rounded-lg border bg-background p-3">
                    <div className="font-medium text-sm">تفويض بدء جلسة Evidence-only واحدة</div>
                    <div><Label>مرجع Runtime UAT</Label><Input className="mt-1" value={pilotSessionStartReference} onChange={(event) => setPilotSessionStartReference(event.target.value)} placeholder="مثال: UAT-AR1-EVIDENCE-ONLY-20260713-001" /></div>
                    <div className="space-y-2 text-sm">
                      <label className="flex items-start gap-3"><Checkbox checked={pilotEvidenceOnlyStartAcknowledged} onCheckedChange={(checked) => setPilotEvidenceOnlyStartAcknowledged(checked === true)} /><span>أقر أن هذه الجلسة Evidence-only ولا تستخدم LLM حتى لو تغير إعداد المزود أثناء الجلسة.</span></label>
                      <label className="flex items-start gap-3"><Checkbox checked={pilotOneQuestionLimitAcknowledged} onCheckedChange={(checked) => setPilotOneQuestionLimitAcknowledged(checked === true)} /><span>أقر أن Runtime UAT يسمح بسؤال داخلي واحد فقط.</span></label>
                      <label className="flex items-start gap-3"><Checkbox checked={pilotRollbackRequiredAcknowledged} onCheckedChange={(checked) => setPilotRollbackRequiredAcknowledged(checked === true)} /><span>أقر بوجوب إلغاء الجلسة ومسحها من الذاكرة بعد التقاط الأدلة.</span></label>
                    </div>
                    <Button onClick={() => startPilotSession.mutate({
                      authorityPreparationId: pilotAuthorityPreparation.id,
                      sessionStartDecision: 'AUTHORIZE_ONE_EPHEMERAL_EVIDENCE_ONLY_UAT_SESSION',
                      uatReference: pilotSessionStartReference,
                      acknowledgeEvidenceOnly: true,
                      acknowledgeOneQuestionLimit: true,
                      acknowledgeRollbackRequired: true,
                    })} disabled={!pilotSessionStartReady || startPilotSession.isPending}><Play className="ml-2 h-4 w-4" />{startPilotSession.isPending ? 'جارٍ بدء الجلسة…' : 'بدء جلسة Evidence-only لمرة واحدة'}</Button>
                    <div className="text-xs leading-6 text-muted-foreground">يُستهلك التحضير مرة واحدة، وتبقى الكتابة والنموذج والنشر محجوبة.</div>
                  </div> : <div className="rounded-lg border bg-background p-3 text-xs leading-6 text-muted-foreground">تم استهلاك التحضير. نفّذ سؤالًا واحدًا ثم ألغِ الجلسة.</div>}
                </div> : null}
              </> : <div className="rounded-lg border border-dashed bg-background p-3 text-sm text-muted-foreground">اختر مادة واحدة أولًا؛ لن تظهر مراجع التحضير قبل ذلك.</div>}
            </div> : null}

            {pilotSession ? <div className="space-y-4 rounded-2xl border border-primary/25 bg-primary/5 p-4">
              <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><div className="font-semibold">5. جلسة AR1 نشطة</div><div className="mt-1 text-xs text-muted-foreground">{pilotSession.id} · تنتهي: {dateLabel(pilotSession.expiresAt)} · {pilotSession.bindingCount} مادة · mode={pilotSession.executionMode} · questions={pilotSession.questionsExecuted ?? 0}/{pilotSession.maxQuestions ?? 1}</div></div><Button variant="outline" onClick={() => rollbackPilot.mutate({ sessionId: pilotSession.id })} disabled={rollbackPilot.isPending}><RotateCcw className="ml-2 h-4 w-4" />إلغاء الجلسة ومسح الذاكرة</Button></div>
              ...
              <div><Label>السؤال البحثي الداخلي</Label><Textarea className="mt-1" value={pilotQuestion} onChange={(event) => setPilotQuestion(event.target.value)} placeholder="اكتب سؤالًا عن النصوص أو العلاقات الظاهرة في corpus المختار…" /><div className="mt-2 flex flex-wrap items-center gap-2"><Button onClick={() => askPilot.mutate({ sessionId: pilotSession.id, question: pilotQuestion })} disabled={!pilotQuestion.trim() || askPilot.isPending || Boolean(pilotResult) || (pilotSession.questionsExecuted ?? 0) >= (pilotSession.maxQuestions ?? 1)}><Send className="ml-2 h-4 w-4" />{askPilot.isPending ? 'جارٍ فحص الأدلة…' : 'تنفيذ السؤال المقيد'}</Button><Badge variant="outline">{pilotSession.executionMode === 'evidence_only_runtime_uat' ? 'Evidence-only مثبت بعقد الجلسة' : (pilotStatus.data?.llmGenerationEnabled ? 'توليد داخلي محكوم' : 'Evidence-only')}</Badge></div></div>
              {pilotResult ? <div className="space-y-3 rounded-lg border bg-background p-4"><div className="flex flex-wrap items-center gap-2"><Badge variant={pilotResult.action === 'answer' ? 'secondary' : 'outline'}>{pilotResult.action}</Badge><Badge variant="outline">ثقة: {pilotResult.confidence}</Badge>{(pilotResult.reasonCodes || []).map((reason: string) => <Badge key={reason} variant="outline">{reason}</Badge>)}</div><pre className="whitespace-pre-wrap font-sans text-sm leading-7">{pilotResult.answer}</pre><div className="text-xs text-muted-foreground">mode=<code>{pilotResult.executionMode}</code> · LLM used=<code>{String(pilotResult.llmGenerationUsed)}</code> · next=<code>{pilotResult.nextAction}</code></div><div className="space-y-2"><div className="font-medium text-sm">الاستشهادات المستعملة</div>{(pilotResult.citations || []).map((citation: any) => <div className="rounded-md border p-3 text-sm" key={citation.citationId}><div className="font-medium">{citation.citationId} · {citation.title}</div><div className="mt-1 text-xs text-muted-foreground">{citation.evidenceTier} · document={citation.documentId} · score={citation.relevanceScore}</div>{citation.sourceUrl ? <a className="mt-1 inline-block text-xs text-primary hover:underline" href={citation.sourceUrl} target="_blank" rel="noopener noreferrer">{citation.sourceUrl}</a> : null}</div>)}</div></div> : null}
            ...
            <div className="space-y-3 border-t pt-4">
              ...
              {pilotTechnicalOpen ? <div className="rounded-xl border bg-muted/20 p-4 text-xs leading-7 text-muted-foreground"><div>AR1 status: <code>{pilotStatusCode || 'unknown'}</code></div><div>C3 evidence expiry: <code>{pilotStatus.data?.c3EvidenceExpiresAt || 'not_available'}</code></div><div>Corpus direct knowledge references: <code>{pilotStatus.data?.corpusResolution?.directC4MaterialReferences ?? 0}</code></div><div>Corpus direct reference-document associations: <code>{pilotStatus.data?.corpusResolution?.directReferenceDocumentAssociations ?? 0}</code></div><div>Deterministic exact matches: <code>{pilotStatus.data?.corpusResolution?.deterministicExactMatches ?? 0}</code></div><div>Rejected ambiguous direct associations: <code>{pilotStatus.data?.corpusResolution?.rejectedAmbiguousDirectAssociations ?? 0}</code></div><div>Rejected fuzzy/ambiguous matches: <code>{pilotStatus.data?.corpusResolution?.rejectedFuzzyOrAmbiguousMatches ?? 0}</code></div><div>Active authority preparations: <code>{pilotStatus.data?.activeAuthorityPreparations ?? 0}</code></div><div>Session start policy: <code>{pilotStatus.data?.sessionStartPolicy || 'not_available'}</code></div></div> : null}
            </div>
          </div>
        </AdminCard>
