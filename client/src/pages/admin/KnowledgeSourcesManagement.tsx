...
export default function KnowledgeSourcesManagement() {
    ...
    onSuccess: (data) => {
      if (data.success) {
        toast.success("✅ نجح الجلب", {
          description: `تم جلب ${data.itemsFetched ?? data.itemsCount ?? 0} عنصر${data.itemsSaved !== undefined ? `، تم حفظ ${data.itemsSaved}` : ""}`,
        });
      } else {
        toast.error("❌ فشل الجلب", {
          description: (Array.isArray((data as any).errors) ? (data as any).errors.join(", ") : data.error) || "حدث خطأ",
        });
      }
      refetch();
    },
  ...
  const handleToggleActive = (id: number, isActive: boolean) => {
    toggleActiveMutation.mutate({ id, isActive: isActive ? 1 : 0 });
  };
  ...
  return (
    <AdminPage>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl admin-page-cleanup">
          <DialogHeader>
