...
export const runtimeQueueLearningCandidateReview = (input: { knowledgeDocumentId: string; reviewerAuthUserId: string; notes?: string | null; }) => callRpc('rpc_queue_learning_candidate_review_v1', { p_knowledge_document_id: input.knowledgeDocumentId, p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_notes: input.notes || null }, 'تعذر إدخال مرشح التعلم إلى مسار المراجعة');
export const runtimeMaterializeResearchLearningCandidate = (input: { toolRunId: string; reviewerAuthUserId: string; notes?: string | null; }) => callRpc('rpc_materialize_research_learning_candidate_v1', { p_tool_run_id: input.toolRunId, p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_notes: input.notes || null }, 'تعذر تحويل تشغيل البحث الوقفي إلى مرشح تعلم محكوم');

export const runtimeRunKnowledgeActivationAudit = (input: { reviewerAuthUserId: string; note?: string; }) => callRpc('rpc_refresh_knowledge_activation_audit_v1', { p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_run_note: input.note || null }, 'تعذر تشغيل تدقيق المعرفة الشامل');
export async function runtimeGetKnowledgeActivationSnapshot() {
  const { data, error } = await client().rpc('rpc_get_knowledge_activation_snapshot_v1');
  if (error) fail('تعذر قراءة حالة تفعيل المعرفة', error);
  return data || { latestRunId: null, summary: { total_documents: 0, review_required: 0 } };
}
export async function runtimeListKnowledgeActivationItems(input?: { runId?: string | null; bucket?: string | null; limit?: number; }) {
  const { data, error } = await client().rpc('rpc_list_knowledge_activation_items_v1', { p_run_id: input?.runId || null, p_bucket: input?.bucket || null, p_limit: Math.max(1, Math.min(input?.limit || 100, 500)) });
  if (error) fail('تعذر قراءة عناصر تفعيل المعرفة', error);
  return { items: data || [], mode: 'knowledge_activation_full_corpus_audit_v1' };
}
...
export const runtimeRunLegacyProvenanceReconstruction = (input: { reviewerAuthUserId: string; activationRunId?: string | null; note?: string; }) => callRpc(
  'rpc_refresh_legacy_provenance_reconstruction_v1',
  { p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_activation_run_id: input.activationRunId || null, p_run_note: input.note || null },
  'تعذر تشغيل إعادة بناء منشأ المعرفة الموروثة'
);
export async function runtimeGetLegacyProvenanceSnapshot() {
  const { data, error } = await client().rpc('rpc_get_legacy_provenance_reconstruction_snapshot_v1');
  if (error) fail('تعذر قراءة حالة إعادة بناء المنشأ', error);
  return data || { latestRunId: null, summary: { total_items: 0, unresolved_items: 0 } };
}
export async function runtimeListLegacyProvenanceItems(input?: { runId?: string | null; state?: string | null; limit?: number; }) {
  const { data, error } = await client().rpc('rpc_list_legacy_provenance_reconstruction_items_v1', { p_run_id: input?.runId || null, p_state: input?.state || null, p_limit: Math.max(1, Math.min(input?.limit || 100, 500)) });
  if (error) fail('تعذر قراءة عناصر إعادة بناء المنشأ', error);
  return { items: data || [], mode: 'legacy_provenance_reconstruction_v1' };
}
export async function runtimeListLegacyProvenanceGroups(input?: { runId?: string | null; limit?: number; }) {
  const { data, error } = await client().rpc('rpc_list_legacy_provenance_groups_v1', { p_run_id: input?.runId || null, p_limit: Math.max(1, Math.min(input?.limit || 100, 500)) });
  if (error) fail('تعذر قراءة مجموعات أدلة المنشأ', error);
  return { groups: data || [], mode: 'legacy_provenance_groups_v1' };
}
export const runtimeRecordLegacyProvenanceDecision = (input: { reviewerAuthUserId: string; itemId: string; decisionCode: string; evidenceReference: string; candidateUrl?: string | null; issuerName?: string | null; notes?: string | null; }) => callRpc(
  'rpc_record_legacy_provenance_resolution_v1',
  { p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_item_id: input.itemId, p_decision_code: input.decisionCode, p_evidence_reference: input.evidenceReference, p_candidate_url: input.candidateUrl || null, p_issuer_name: input.issuerName || null, p_notes: input.notes || null },
  'تعذر تسجيل قرار منشأ المعرفة'
);
export async function runtimeListDuplicateCitationReviewTasks(input?: { limit?: number; }) {
  const { data, error } = await client().rpc('rpc_list_duplicate_citation_review_task_groups_v1', { p_limit: Math.max(1, Math.min(input?.limit || 100, 500)) });
  if (error) fail('تعذر قراءة ازدواج مهام الاستشهاد', error);
  return { groups: data || [], mode: 'duplicate_citation_review_task_groups_v1' };
}
export const runtimeReconcileDuplicateCitationReviewTask = (input: { reviewerAuthUserId: string; keepTaskId: string; cancelTaskId: string; rationale: string; }) => callRpc(
  'rpc_reconcile_duplicate_citation_review_task_v1',
  { p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_keep_task_id: input.keepTaskId, p_cancel_task_id: input.cancelTaskId, p_rationale: input.rationale },
  'تعذر تسوية ازدواج مهام الاستشهاد'
);


export const runtimeRunLegacyProvenanceReconstructionV11 = (input: { reviewerAuthUserId: string; activationRunId?: string | null; note?: string; }) => callRpc(
  'rpc_refresh_legacy_provenance_reconstruction_v1_1',
  { p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_activation_run_id: input.activationRunId || null, p_run_note: input.note || null },
  'تعذر تشغيل إعادة بناء المنشأ V1.1'
);
export async function runtimeGetLegacyProvenanceSnapshotV11() {
  const { data, error } = await client().rpc('rpc_get_legacy_provenance_reconstruction_snapshot_v1_1');
  if (error) fail('تعذر قراءة حالة إعادة بناء المنشأ V1.1', error);
  return data || { latestRunId: null, summary: { total_items: 0, deterministic_lineage_items: 0, review_required_items: 0, test_artifact_items: 0 } };
}
export async function runtimeListLegacyProvenanceItemsV11(input?: { runId?: string | null; state?: string | null; limit?: number; }) {
  const { data, error } = await client().rpc('rpc_list_legacy_provenance_reconstruction_items_v1_1', { p_run_id: input?.runId || null, p_state: input?.state || null, p_limit: Math.max(1, Math.min(input?.limit || 100, 500)) });
  if (error) fail('تعذر قراءة عناصر إعادة بناء المنشأ V1.1', error);
  return { items: data || [], mode: 'legacy_provenance_reconstruction_v1_1' };
}
export async function runtimeListLegacyProvenanceGroupsV11(input?: { runId?: string | null; limit?: number; }) {
  const { data, error } = await client().rpc('rpc_list_legacy_provenance_groups_v1_1', { p_run_id: input?.runId || null, p_limit: Math.max(1, Math.min(input?.limit || 100, 500)) });
  if (error) fail('تعذر قراءة مجموعات أدلة المنشأ V1.1', error);
  return { groups: data || [], mode: 'legacy_provenance_groups_v1_1' };
}
export async function runtimeGetLegacyProvenanceDecisionContextV11(input: { itemId: string; }) {
  const item = await readOne(
    'knowledge_provenance_reconstruction_items_v1_1',
    input.itemId,
    'id,reference_document_id,provenance_state',
  );
  if (!item) throw new Error('عنصر منشأ المعرفة المطلوب غير موجود.');
  return {
    itemId: String(item.id),
    referenceDocumentId: item.reference_document_id ? String(item.reference_document_id) : null,
    provenanceState: String(item.provenance_state || ''),
  };
}
export const runtimeRecordLegacyProvenanceDecisionV11 = (input: { reviewerAuthUserId: string; itemId: string; decisionCode: string; evidenceReference: string; candidateUrl?: string | null; issuerName?: string | null; notes?: string | null; }) => callRpc(
  'rpc_record_legacy_provenance_resolution_v1_1',
  { p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_item_id: input.itemId, p_decision_code: input.decisionCode, p_evidence_reference: input.evidenceReference, p_candidate_url: input.candidateUrl || null, p_issuer_name: input.issuerName || null, p_notes: input.notes || null },
  'تعذر تسجيل قرار منشأ المعرفة V1.1'
);
export async function runtimeListDuplicateCitationReviewTasksV11(input?: { limit?: number; }) {
  const { data, error } = await client().rpc('rpc_list_duplicate_citation_review_task_groups_v1_1', { p_limit: Math.max(1, Math.min(input?.limit || 100, 500)) });
  if (error) fail('تعذر قراءة ازدواج مهام الاستشهاد V1.1', error);
  return { groups: data || [], mode: 'duplicate_citation_review_task_groups_v1_1' };
}
export const runtimeReconcileDuplicateCitationReviewTaskV11 = (input: { reviewerAuthUserId: string; keepTaskId: string; cancelTaskId: string; rationale: string; }) => callRpc(
  'rpc_reconcile_duplicate_citation_review_task_v1_1',
  { p_reviewer_auth_user_id: reviewerId(input.reviewerAuthUserId), p_keep_task_id: input.keepTaskId, p_cancel_task_id: input.cancelTaskId, p_rationale: input.rationale },
  'تعذر تسوية ازدواج مهام الاستشهاد V1.1'
);
